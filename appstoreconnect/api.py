...
class Api:
    def __init__(
        self, key_id, key_file, issuer_id, submit_stats=True, timeout=None, proxy=None
    ):
        self._token = None
        ...
        for attribute in resource.attributes:
            if attribute in args and args[attribute] is not None:
                if type(args[attribute]) == list:
                    value = list(
                        map(
                            lambda e: e.name if isinstance(e, Enum) else e,
                            args[attribute],
                        )
                    )
    ...
    def _api_call(self, url, method=HttpMethod.GET, post_data=None):
        ...
        try:
            if method == HttpMethod.GET:
                ...
                r = requests.get(
                    url, headers=headers, timeout=self.timeout, proxies=proxies
                )
    ...
    def _submit_stats(self, event_type):
        ...
        payload = {
            ...
            "parameters": {
                ...
                "issuer_id_hash": hashlib.sha1(
                    self.issuer_id.encode()
                ).hexdigest(),  # send anonymized hash
            },
        }
    ...
    def download_finance_reports(
        self, filters=None, split_response=False, save_to=None
    ):
